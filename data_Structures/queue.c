...
static bool queue_resize(Queue *q, size_t new_capacity) {
	...
	// 🔒 Evita desbordamiento en la petición a malloc
	if (new_capacity > SIZE_MAX / q->element_size) return false;
...
bool queue_enqueue(Queue *q, const void *element) {
	...
	if (q->count == q->capacity) {
		...
		if (q->capacity > SIZE_MAX / 2) return false;
...
bool queue_dequeue(Queue *q, void *out) {
	...
	// 1. Copiar al buffer del usuario si se proporciona
	if (out) memcpy(out, src, q->element_size);

	// 2. 🔥 Llamar destructor sobre el elemento que se elimina del buffer interno
	if (q->destructor) {
			q->destructor(src);
	}
