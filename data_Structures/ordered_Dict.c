...
/*
  ...
  (ordered_dict_remove), you need to jump directly
 ...
 */
#define ORDERED_DICT_LOAD_FACTOR 0.75

static size_t ordered_dict_hash(const ordered_Dict *d, const void *key) {
...
static bool ordered_dict_resize(ordered_Dict *d, size_t new_capacity) {
  ...
  for (size_t i = 0; i < old_capacity; ++i) {
    if (old_entries[i].occupied && !old_entries[i].deleted) {
      ...
      ordered_dict_put(d, old_entries[i].key, old_entries[i].value);
      
      // Liberar memoria de las copias viejas (ordered_dict_put hizo nuevas)
...
ordered_Dict *ordered_dict_create(size_t key_size, size_t value_size,
  ...
  ordered_Dict *d = malloc(sizeof(ordered_Dict));
...
static void free_insert_order_list(ordered_Dict *d) {
...
bool ordered_dict_put(ordered_Dict *d, const void *key, const void *value) {
...
static DictEntry *ordered_dict_find_entry(const ordered_Dict *d, const void *key) {
...
bool ordered_dict_get(const ordered_Dict *d, const void *key, void *out_value) {
  DictEntry *e = ordered_dict_find_entry(d, key);
  if (!e) return false;
  if (out_value) memcpy(out_value, e->value, d->value_size);
  return true;
}

bool ordered_dict_remove(ordered_Dict *d, const void *key) {
  DictEntry *e = ordered_dict_find_entry(d, key);
  if (!e) return false;
...
bool ordered_dict_contains(const ordered_Dict *d, const void *key) {
  return ordered_dict_find_entry(d, key) != NULL;
}

size_t ordered_dict_size(const ordered_Dict *d) {
  return d ? d->size : 0;
}
