// Compilar: gcc tests.c ../ordered_Dict.c ../stack.c ../queue.c -o tests
...
static size_t int_hash(const void* key) {
    ...
    return (size_t)(x * UINT32_C(0x9E3779B1)); // Constante de Knuth
}
static int int_compare(const void* a, const void* b) {
  return (*(const int*)a == *(const int*)b) ? 0 : 1;
}
/* El multiplicador de Knuth se usa para distribuir los bits en
...
static void test_dict_create_destroy(void) {
  ...
  ASSERT(ordered_dict_size(d) == 0, "Tamaño inicial es 0");
...
static void test_dict_basic_ops(void) {
  ...
  ASSERT(ordered_dict_put(d, &k1, &v1), "Insertar clave 10");
  ASSERT(ordered_dict_put(d, &k2, &v2), "Insertar clave 20");
  ASSERT(ordered_dict_size(d) == 2, "Tamaño = 2");
  ASSERT(ordered_dict_contains(d, &k1), "Contiene clave 10");
  ASSERT(ordered_dict_get(d, &k1, &out) && out == 100, "Recuperar valor de 10");
  ASSERT(ordered_dict_get(d, &k2, &out) && out == 200, "Recuperar valor de 20");
  ASSERT(!ordered_dict_get(d, &(int){99}, &out), "Clave inexistente retorna false");
...
static void test_dict_update_with_destructor(void) {
  ...
  ordered_dict_put(d, &k, &v_old);
  ASSERT(destroy_count == 0, "Sin llamadas a destructor en inserción");

  ASSERT(ordered_dict_put(d, &k, &v_new), "Actualizar valor existente");
  ASSERT(ordered_dict_size(d) == 1, "Tamaño no cambia al actualizar");
  ASSERT(ordered_dict_get(d, &k, &out) && out == 500, "Valor actualizado correctamente");
...
static void test_dict_remove_and_order(void) {
  ...
  for(int i = 0; i < 4; ++i) ordered_dict_put(d, &keys[i], &vals[i]);

  ASSERT(ordered_dict_remove(d, &keys[1]), "Eliminar clave 2 (intermedia)");
  ASSERT(ordered_dict_size(d) == 3, "Tamaño = 3 tras eliminar");
  ASSERT(!ordered_dict_contains(d, &keys[1]), "Clave eliminada ya no existe");

  /* Verificar orden: 1 → 3 → 4 */
  DictIterator* it = ordered_dict_iterator_create(d);
  int expected_k[] = {1, 3, 4}, expected_v[] = {10, 30, 40};
  int i = 0, cur_k, cur_v;
  while(ordered_dict_iterator_next(it, &cur_k, &cur_v)) {
...
static void test_dict_resize(void) {
  ...
  for(int i = 1; i <= 10; ++i) {
      int k = i, v = i * 100;
      ASSERT(ordered_dict_put(d, &k, &v), "Insertar para forzar resize");
  }
  ASSERT(ordered_dict_size(d) == 10, "Tamaño correcto tras resize");

  /* Verificar integridad post-rehash */
  for(int i = 1; i <= 10; ++i) {
      int k = i, out;
      ASSERT(ordered_dict_get(d, &k, &out) && out == i * 100, "Datos intactos tras resize");
  }
  ...
  for(int i = 1; i <= 10; ++i) {
      int k, v;
      ordered_dict_iterator_next(it, &k, &v);
...
static void test_dict_null_safety(void) {
  ...
  ASSERT(!ordered_dict_get(NULL, NULL, NULL), "ordered_dict_get(NULL) seguro");
  ASSERT(!ordered_dict_remove(NULL, NULL), "ordered_dict_remove(NULL) seguro");
  ASSERT(!ordered_dict_put(NULL, NULL, NULL), "ordered_dict_put(NULL) retorna false");
...
static void test_mixed_operations(void) {
  ...
  /* Ordered Dict con valores que son punteros a structs simulados */
  typedef struct { int id; char label[8]; } Item;
  destroy_count = 0;

  ordered_Dict* d = ordered_dict_create(sizeof(int), sizeof(Item), int_compare, int_hash, 
  ...
  ordered_dict_put(d, &k_upd, &v_upd);
  ASSERT(destroy_count == 1, "Destructor llamado al actualizar");

  /* Eliminar otro */
  ordered_dict_remove(d, &(int){0});
  ...
  while(ordered_dict_iterator_next(it, &tmp_k, &tmp_v)) count++;
...
static void test_empty_structures(void) {
  ...
  ordered_Dict* d = ordered_dict_create(sizeof(int), sizeof(int), int_compare, int_hash, NULL, NULL);
  ASSERT(!ordered_dict_get(d, &(int){1}, &out), "Get en ordered dict vacío falla");
  ASSERT(!ordered_dict_remove(d, &(int){1}), "Remove en ordered dict vacío falla");

  DictIterator* it = ordered_dict_iterator_create(d);
  ASSERT(!ordered_dict_iterator_next(it, NULL, NULL), "Iterador en ordered dict vacío retorna false");
  ordered_dict_iterator_destroy(it);
  ordered_dict_destroy(d);
}
